...
extracted_characters_folder = "../../data/character_extraction/etl_all_processed/" + mode
file_of_dict = '../../data/jpn_ipadict/merge2.csv'
path_for_save_result = "../../data/generate_sentences/imgs_from_hw_jpn/" + mode
background_path = '../../data/generate_sentences/background_images/'
...
def random_img(duck):
    ...
    for ind, i in enumerate(dictwords):
        charfolder = os.listdir(os.path.join(extracted_characters_folder, i))
    ...
    if (max_total_width + 20) > (max_max_height + 10):
        if (500 / (max_total_width + 20 )) <= 0.4:
            rate = (500 / (max_total_width + 20 ))
        else:
            rate = np.random.uniform(0.4, (500 / (max_total_width + 20 )))
    else:
        if (500 / (max_max_height + 10)) <= 0.4:
            rate = (500 / (max_max_height + 10))
        else:
...
>>>>>>> debug
        ...
        return
    elif 500 - (((max_height + max_height2 + max_height3) + 30) * rate) < (
        (max_height3 + 10) * rate):
        return
...
>>>>>>> debug
    ...
    y_p = np.random.randint(0, 500 - ((
        (max_height + max_height2 + max_height3) * rate) + 30))
...
>>>>>>> debug
    ...
    for ind, i in enumerate(ims):
        ...
        i = i.transform((new_w, new_h), Image.AFFINE,
                        (1, -m, int(orin_h * m)
                         if m < 0 else 0, -n, 1, int(orin_w * n)
                         if n < 0 else 0), Image.BILINEAR)
        ...
        if orin_h < (0.4 * max(heights)):
            y_b = 500 - int((max_height * 0.7 * rate) + y_gap + (
                max_height2 * rate) + (max_height3 * rate) + 10)
        else:
            y_b = 500 - int((i.size[1] * rate) + y_gap + (max_height2 * rate) +
                            (max_height3 * rate) + 10)
    ...
    for ind, i in enumerate(ims2):
        ...
        i = i.transform((new_w, new_h), Image.AFFINE,
                        (1, -m2, int(orin_h * m2)
                         if m2 < 0 else 0, -n2, 1, int(orin_w * n2)
                         if n2 < 0 else 0), Image.BILINEAR)

        i = i.rotate(ang2, expand=True)
        #set random gap
        y_gap = np.random.randint(0, 5)
        x_gap = np.random.randint(int(2 * rate), int(10 * rate))
        ...
        if orin_h < (0.4 * max(heights2)):
            y_b = 500 - int((max_height2 * 0.7 * rate) + (max_height3 * rate) +
                            10 + y_gap)
        else:
            y_b = 500 - int((i.size[1] * rate) + (max_height3 * rate) + 10 +
                            y_gap)
        ...
        #center of image
        txt2.append(
            (dictwords2[ind] + " " + i_x + " " + i_y + " " + i_w + " " + i_h))
    ...
    for ind, i in enumerate(ims3):
        ...
        i = i.transform((new_w, new_h), Image.AFFINE,
                        (1, -m3, int(orin_h * m3)
                         if m3 < 0 else 0, -n3, 1, int(orin_w * n3)
                         if n3 < 0 else 0), Image.BILINEAR)

        i = i.rotate(ang3, expand=True)
        #set random gap
        y_gap = np.random.randint(0, 5)
        x_gap = np.random.randint(int(2 * rate), int(10 * rate))
        ...
        #center of image
        txt3.append(
            (dictwords3[ind] + " " + i_x + " " + i_y + " " + i_w + " " + i_h))
    ...
    if len(str(duck)) < 7:
        ...
        name = str(duck)
    with codecs.open(
            os.path.join(path_for_save_result, "Annotations/%s.txt" %
                         (name)), "w", 'utf-8') as output:
        output.write(txt)
...
def shuffle_words_img(goose):
    ...
    for duck in range(0, int(len(words) / 3)):
        ...
        remaining_time = int(elapsed_time * (len(words) / 3 - duck) / (duck+1))
        _printProgress(duck, int(len(words) / 3),
            prefix = 'Generating ({}/{})'.format(duck, int(len(words) / 3)),
...
with codecs.open(
        os.path.join(path_for_save_result, "charlist.txt"), "w",
        'utf-8') as output:
